...
load('NASDAQ_30step_multifeature_data1.mat');
...
function particles = update_particles(particles, global_best, param_ranges)
    ...
    for i = 1:length(particles)
        ...
        % Construct global best parameter vector
        global_best_vector = [...
            global_best.Nr, ...
            global_best.Nl, ...
...
epsilon = 1e-6;
percentage_error = abs_error ./ (abs(target_test) + epsilon);

% Core metrics including R-squared
...
final_r2 = calculate_r2(test_output, target_test);

% Variance metrics
mse_var = var(squared_error(:));
mae_var = var(abs_error(:));
mape_var = var(percentage_error(:)) * 100^2;
...
disp(['Test R²:      ', num2str(final_r2, '%.4f')]);  % R-squared metric
disp(['Training time:    ', num2str(train_time, '%.3f'), ' seconds']);
disp(['Testing time:    ', num2str(test_time, '%.3f'), ' seconds']);

%% 100 iterations for statistical evaluation
num_runs = 100;

% Preallocate storage including R-squared
...
% Statistical results including R-squared
...
figure;
plot(target_test(50:end), 'r', 'LineWidth', 1.2);
hold on;
plot(test_output(50:end), 'b--', 'LineWidth', 1.2);
legend('Actual Values', 'Predicted Values', 'Location', 'best');
title('NASDAQ 30-Step Prediction: Actual vs Predicted');
xlabel('Time Step');
